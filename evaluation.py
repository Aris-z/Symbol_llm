...
def main():
    for iter_num in range(1):
        ...
        for part in range(1, part_num + 1):
            with open(f"{PATH_TO_CODE}/mbpp_full_llama2chat_part{part}_iter{iter_num}.json", "r") as f:
...
def test():
    for iter_num in range(0, 10):
        data_code = []
        with open(f"{PATH_TO_CODE}/mbpp_full_llama2chat_sft_iter{iter_num}_code_agent.jsonl", "r") as f:
            for line in f:
                data_code.append(json.loads(line))
        acc = []

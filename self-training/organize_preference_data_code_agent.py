...
def main():
    ...
    for iter_idx in range(args.cur_iter+1):
        ...
        for i in range(1, PART_NUM + 1):
            if iter_idx == 0:
                part_name = f"part{i}"
                score = np.load(f"score_memory/code_agent/{args.task_prefix}/scores_{args.task_prefix}_{part_name}_iter{iter_idx}.npy").tolist()
                with open(f"score_memory/code_agent/{args.task_prefix}/{args.task_prefix}_{part_name}_iter{iter_idx}.json",'r') as file:
                    data = json.load(file)
                scores += score
                candidates += data
            else:
                part_name = f"part{i}"
                score = np.load(f"score_memory/code_agent/{args.task_prefix}/scores_{args.task_prefix}_{part_name}_iter{iter_idx}.npy").tolist()
                with open(f"new_generated_data/code_agent/{args.task_prefix}_{part_name}_iter{iter_idx}.json", 'r') as file:
                    data = json.load(file)
                scores += score
                candidates += data

                # load self-repaired samples
                score_repaired = np.load(f"score_memory/code_agent/{args.task_prefix}/scores_{args.task_prefix}_{part_name}_iter{iter_idx}_repaired.npy").tolist()
                with open(f"new_generated_data/code_agent/{args.task_prefix}_{part_name}_iter{iter_idx}_repaired.json", 'r') as file:
    ...
    with jsonlines.open(f"open-instruct/data/{args.task_prefix}_sft_iter{args.cur_iter}_code_agent.jsonl",'w') as file:
